...
$fname="prod_to_cat.csv";
$arr= array();
$tmp= array();
if (($handle = fopen($fname, "r")) !== FALSE) {
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                
                if (!in_array($data[0], $tmp)) {
                            $tmp[]=$data[0];
 ...
 $fp = fopen('prod.csv', 'ab');
 $sky=999;
foreach ($arr as $key => $value) {
    $a= array( $value[0], $value[2],  $value[1], $sky++,'','','','','','', 100,
      'model' ,'manuf',  '/upload/iblock/644/644d1951512445e4efa219ec4f4f4a59.jpg', 'yes'
      , 999,  0, '2016-08-11 18:31:50', '2016-09-14 18:19:19','2016-08-11',
        '4,00',  'kg',  2222, 4, 4, 'cm',  true  ,0, '',  'Террасная доска Тик гладкая/гладкая 19x120х1200-2400', 'Древесина тик', 'гладкая поверхность, без покрытия'      ,5, 0,'','', ''  ,1 ,true,  1);       
...
die();
   ...
   $fp = fopen('prod_to_cat_ok.csv', 'ab');
foreach ($arr as $key => $value) {
...
for ($i=0; $i <$total ; $i++) { 
  ...
  $c=count($all[$i]['bread']);
     if ($deep>=$c) { // продукт
     	...
     	/*	$firephp->fb($c);
     		$firephp->fb($all[$i]);*/
 $prod_arr[]=  array('prod_id'=>$all[$i]['id'],'cat_id' => $p_id);
...
/*	$firephp->fb($value);
  die();*/
      log_compere(
        array($value['id'],$value['p_id'],$value['name_key'],'true',1,20,'','2009-01-03 21:08:57','2009-01-03 21:08:57','','','','',0,'','true'),'categori.csv','ab');       
}
