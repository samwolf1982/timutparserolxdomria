...
$file = fopen("res1/cat_urls.csv","w");

foreach ($bread1a as $key => $value) {
  /*      $tde=pq($value)->html();
   $td[$i++] =  iconv(mb_detect_encoding($tde, mb_detect_order(), true), "UTF-8", $tde); */
       $breadh=pq($value)->find('a') ->attr('href');
 $txt=pq($value)->text(); 

$number=pq($value)->find('span')->text();
$firephp->fb(array('url'=> $breadh,'name'=>$txt,'number'=>$number));
 fputcsv($file,array('url'=> $breadh,'name'=>$txt,'number'=>$number));

}
fclose($file);

/*    for ($i=0; $i <count($bread1a);  $i++) { 
        $td=pq($bread1a[$i])->html();
   $td[$i] =  iconv(mb_detect_encoding($td, mb_detect_order(), true), "UTF-8", $td); 

    }*/


/*    $stmt = $db->prepare('UPDATE `oc_category_description` SET `description`= ? , `description2` = ? WHERE `category_id` = ? ');
$stmt->execute(array($td[0],$td[1],$_POST['param'][0]));
            $firephp->fb(array($td[0],$td[1],$_POST['param'][0]));*/
...
$id_to_filters = array('id' =>$_POST['param'][0] ,'filters'=> $filters );
/*echo json_encode( $id_to_filters);*/

if (count($filters>0)) {
$file = 'res/id_to_filters.txt';
}else{
 //   ошибка нету фильтров
 $file = 'res/erro_filters.txt';
$id_to_filters = array('id' =>$_POST['param'][0]);

}

$person =json_encode($id_to_filters).PHP_EOL;
