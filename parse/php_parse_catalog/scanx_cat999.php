...
$file = fopen("res6/errrofile.csv","ab");
 fputcsv($file,$res1);
fclose($file);
...
$mail=pq($bread1a)->text();

    /*$firephp->fb(array($name,$mail));*/



$file = fopen("res6/s6_ready.csv","ab");

$res = array($mail ,$mail, $name,'','',$_POST['param'][0]);
...
die();
 ...
 foreach ($bread1a as $key => $value) {
      ...
      $url=pq($value)->find('a')->attr('onclick');

      $url=str_replace("window.open('", '', $url);
      $url=str_replace(",'singleSupplier','width=500,height=500');return false;", '', $url);

     
      if (!empty($url)) {
       $url=$GLOBALS['curent_host_fullwslahs'].$url;
      }


      

$file = fopen("res6/all_urls.csv","ab");
