...
try {
  ...
  file_put_contents('error_id.txt', $_POST['param'][1].PHP_EOL, FILE_APPEND | LOCK_EX);
}
...
$b1=array();
       $bread1='.breadcrumbs > ul:nth-child(2) > li';
  ...
  foreach ($bread1a as $key => $value) {
   ...
   /*   $b1[]=trim(pq($value)->find('a')->attr('href'));*/
      $b1[trim(pq($value)->text())]=pq($value)->find('a')->attr('href');
  }
...
file_put_contents('cat.txt', $main_arr.PHP_EOL, FILE_APPEND | LOCK_EX);
    ...
    $stmt = $db->prepare("INSERT INTO `a_cat`(`txt`) VALUES (:val1)");
        $stmt->bindParam(':val1', $main_arr);
        $result = $stmt->execute();
