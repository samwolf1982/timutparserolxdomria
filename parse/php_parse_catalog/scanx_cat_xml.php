...
$_POST['param']='http://www1.co.il/sitemap.xml';
if (isset($_POST['param'])) {
  # code...
/*   $firephp = FirePHP::getInstance(true);
  $firephp->fb(array($_POST['param'])); die();*/
...
$file = fopen("res3/log_error.csv","ab");
 fputcsv($file,$res1);
fclose($file);
...
$b1=array();//*[@id="content"]/table/tbody
       $bread1='#content > table > tbody > tr';
/*       content > table > tbody > tr:nth-child(1) > td:nth-child(1) > a*/
$bread1a=$document->find($bread1);

/* $t=pq($bread1a)->text();*/
 $t=count($bread1a);
           $firephp->fb(array('ok', $t));  die();
          $c=count($bread1a);
           $firephp->fb($c);
       foreach ($bread1a as $key => $value) {
         $url=pq($value)->find("td:eq(1) a")->attr('href');
               $firephp->fb($url);
    die(); 
     $b1[]=$url;
       }
...
$file = fopen("res2/s2_ready.csv","ab");
 fputcsv($file,$res);
fclose($file);
